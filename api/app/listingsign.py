...
LISTING_FIELDS = ("make", "model", "year", "price_kes", "mileage_km",
                  "transmission", "fuel", "condition", "body_type", "location", "image_url")
...
def create_draft(*, owner_id: str, fields: dict, draft_id: str | None = None) -> ListingDraft:
    return ListingDraft(
...
def verify_signed_draft(secret: str, signed: dict) -> ListingDraft:
    ...
    payload = {k: signed[k] for k in ("draft_id", "owner_id", "fields", "expires_at") if k in signed}
    if len(payload) != 4 or not isinstance(payload["fields"], dict):
